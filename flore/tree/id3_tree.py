...
class ID3:
    def __init__(self, features, X, y, max_depth=2, min_num_examples = 1, prunning=True, th= 0.0001):
        ...
        self.features = features # lista de caracteristicas. tiene que ser de las mismas que X y ademas ordenadas por el indice ambas. No puede estar la clase.
    ...
    def partial_fit(self, X, y, actual_tree, actual_depth, borradas):
        ...
        num_cases = X.shape[0]
        num_features = len(self.features)-len(borradas)
    ...
    def activateRules(self, tree, rules, conditions, instance, instance_idx, fuzzy_set, df_categorical_columns, threshold, verbose):
        if not tree.is_leaf:
            for i in range(len(tree.splits)):
                ...
                if feat in df_categorical_columns and instance[feat] == branch:
                    self.activateRules(tree.childlist[i], rules, conditions + [(feat, branch, 1)], instance, instance_idx, fuzzy_set, df_categorical_columns, threshold, verbose)
                elif feat not in df_categorical_columns:
                    pert = fuzzy_set[feat][branch][instance_idx]
                    if pert > threshold:
                        self.activateRules(tree.childlist[i], rules, conditions + [(feat, branch, pert)], instance, instance_idx, fuzzy_set, df_categorical_columns, threshold, verbose)
        else:
            if verbose:
    ...
    def explainInstance(self, instance, instance_idx, fuzzy_set, df_categorical_columns, threshold=0.01, verbose=False):
        rules = []
        self.activateRules(self.tree, rules, [], instance, instance_idx, fuzzy_set, df_categorical_columns, threshold, verbose)
        return rules
